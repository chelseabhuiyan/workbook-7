...
SELECT (Salary) AS TotalSalaries
FROM employees;
...
SELECT AVG(UnitPrice) AS AverageItem
FROM Products;
...
supplier ID of each supplier and the number of items they supply? You can answer this query by only looking at the Products table. */
SELECT SupplierID, COUNT(*) AS NumberOfProducts
FROM Products
GROUP BY SupplierID
HAVING COUNT(*) >= 5;
